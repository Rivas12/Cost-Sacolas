...
def init_configuracoes():
    ...
    if count == 0:
        cursor.execute('INSERT INTO configuracoes (id, margem, outros_custos, tema, notificacoes, perdas_calibracao_un, valor_silk) VALUES (1, 0, 0, "Escuro", 0, 0, 0)')
    conn.commit()
    conn.close()
...
def get_configuracoes():
    ...
    conn.close()
    if not row:
        return {'margem': 0.0, 'outros_custos': 0.0, 'tema': 'Escuro', 'notificacoes': 0, 'perdas_calibracao_un': 0, 'valor_silk': 0.0}
    return {
...
def update_configuracoes(margem=None, outros_custos=None, tema=None, notificacoes=None, perdas_calibracao_un=None, valor_silk=None):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    # Monta set dinâmico
    fields = []
